...
WHEN NOT TO USE: Do not use if the SMB was returned from find_business within the last 24 hours \xE2\u20AC\u201D those results are already verified.
COST: $0.02 per_call
LATENCY: ~500ms`,
        inputSchema: {
          type: "object",
      ...
      {
        name: "send_message",
        description: `Cost: $0.02/send (requires Developer token \u2014 get one at https://agent-broker-edge.basil-agent.workers.dev/checkout). Send a message on behalf of an agent's user or an SMB across SMS, email, or voice. Five message types: transactional, reminder, follow_up, notification, marketing. Every send routes through a non-bypassable compliance gate (TCPA, GDPR, CASL, PDPL across 22 jurisdictions) that enforces opt-in consent for marketing/promotional content \xE2\u20AC\u201D marketing without recorded consent is rejected at runtime with a structured compliance_violation receipt. Channel is abstracted: specify intent and recipient; the service selects and falls back across channels.
...
WHEN NOT TO USE: Do NOT use for OTPs or critical transactional confirmations \xE2\u20AC\u201D use send_transactional_confirmation. Do NOT attempt to send marketing without a consent_record_id pointing at a real opt-in \xE2\u20AC\u201D the gate will reject the send and log a compliance_violation. Do NOT attempt bulk / list-based / drip / cold outreach \xE2\u20AC\u201D those are out of scope and the rate limiter will throttle abuse.
...
EXECUTION: sync_fast (use get_outcome to retrieve result)`,
      ...
      {
        name: "capture_lead",
        description: `Cost: $0.008/call (requires Developer token \u2014 get one at https://agent-broker-edge.basil-agent.workers.dev/checkout). Structured intake of a prospect into an SMB's funnel with validation, enrichment hooks, and deduplication. Inserts into the SMB's CRM or direct-booking pipeline if available.
...
WHEN NOT TO USE: Do not use for confirmed bookings \xE2\u20AC\u201D use schedule_appointment. Do not use for bulk list imports.
COST: $0.05 per_lead
LATENCY: ~600ms
EXECUTION: sync_fast (use get_outcome to retrieve result)`,
        inputSchema: {
      ...
      },
      {
        ...
        description: `Cost: $0.015/call (requires Developer token \u2014 get one at https://agent-broker-edge.basil-agent.workers.dev/checkout). Availability lookup, hold, confirm, reschedule, or cancel appointments with an SMB. Routes through the SMB's native booking system if available, falls back to voice AI or web form.
...
WHEN NOT TO USE: Do not use for bulk scheduling. Do not use without a verified SMB \xE2\u20AC\u201D call find_business and verify_business first if needed.
...
WHEN TO USE: Use when an SMB needs inbound message triage \xE2\u20AC\u201D classifying incoming contact-form submissions, SMS replies, voicemails, or email inquiries.
...
EXAMPLE USER QUERIES THAT MATCH THIS TOOL:
  user: "I'm stuck \xE2\u20AC\u201D get a human at smb_xyz to call me back"
...
EXECUTION: async_by_default (use get_outcome to retrieve result)`,
      ...
      },
      {
        ...
        description: `Turn ANY public booking URL (Cal.com, Calendly, Doctolib, Booksy, Fresha, OpenTable, Setmore, Square, Acuity, Schedulista, Squarespace, BookMyCity) into a callable smb_id you can immediately use with schedule_appointment, send_message, or capture_lead. Idempotent \xE2\u20AC\u201D calling twice returns the same smb_id.
...
WHEN NOT TO USE: Do not use if the user only describes a business by name without a URL \xE2\u20AC\u201D call find_business instead. Do not use for arbitrary websites that are not on the supported booking-platform list (use /supply/platforms to see all 12).
COST: $0.005 per_call
LATENCY: ~600ms`,
        inputSchema: {
          type: "object",
      ...
      {
        name: "call_business",
        description: "Cost: $0.10/call (requires Developer token \u2014 get one at https://agent-broker-edge.basil-agent.workers.dev/checkout). Place a conversational voice-AI phone call to a business on a consumer's behalf and return a structured answer. THE differentiated capability: reach the ~60M long-tail SMBs that have NO API and NO booking page \xE2\u20AC\u201D only a phone number. An AI agent cannot pick up a phone and hold a conversation; this tool does. Give a plain-language objective; the voice AI navigates the call and extracts the answer. Business-directed (B2B), far less restricted than calling consumers \xE2\u20AC\u201D but the compliance gate still enforces recording consent per jurisdiction. Async: returns a call handle; poll get_outcome for the transcript + extracted fields.\n\nWHEN TO USE: Use when the target business has NO booking URL and NO API \xE2\u20AC\u201D only a phone number \xE2\u20AC\u201D and the consumer asked the agent to reach them (e.g. 'call this plumber and ask if they can come Tuesday', 'ask the salon if they take walk-ins this afternoon'). Also use to confirm details a booking page doesn't expose (real-time availability, custom quotes).\nWHEN NOT TO USE: Do NOT use when the business has a booking URL \xE2\u20AC\u201D use import_booking_url + schedule_appointment (cheaper, faster, deterministic). Do NOT use for calls to consumers/individuals (this tool is for reaching businesses). Do NOT use for marketing or telemarketing \xE2\u20AC\u201D the compliance gate and the B2B-only framing reject that.\nCOST: $0.5 per_call\nLATENCY: ~45000ms\nEXECUTION: async_by_default (use get_outcome to retrieve result)",
        inputSchema: {
          type: "object",
          required: [
            "objective"
