...
COST: $0.02 per_call
LATENCY: ~500ms`, inputSchema: { type: "object", required: ["smb_id"], properties: { smb_id: { type: "string" }, capability_to_verify: { type: "string" } } }, annotations: { title: "Verify Business", readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false } }, { name: "send_message", description: `Send a message on behalf of an agent's user or an SMB across SMS, email, or voice. Five message types: transactional, reminder, follow_up, notification, marketing. Every send routes through a non-bypassable compliance gate (TCPA, GDPR, CASL, PDPL across 22 jurisdictions) that enforces opt-in consent for marketing/promotional content \u2014 marketing without recorded consent is rejected at runtime with a structured compliance_violation receipt. Channel is abstracted: specify intent and recipient; the service selects and falls back across channels.
...
LATENCY: ~800ms
EXECUTION: sync_fast (use get_outcome to retrieve result)`, inputSchema: { type: "object", required: ["recipient", "message_type", "content"], properties: { recipient: { type: "object", required: ["id_type", "id_value"], properties: { id_type: { type: "string", enum: ["phone", "email", "smb_id", "customer_id"] }, id_value: { type: "string" }, country_code: { type: "string", description: "ISO 3166-1 alpha-2, required for compliance routing" } } }, message_type: { type: "string", description: "Intent tag for the message. Five permitted types. 'marketing' is allowed only when paired with a valid consent_record_id; the compliance gate verifies the consent at send time and rejects (compliance_violation receipt) if it's missing, expired, or revoked.", enum: ["transactional", "marketing", "reminder", "follow_up", "notification"] }, content: { type: "object", required: ["body"], properties: { body: { type: "string" }, subject: { type: "string", description: "For email channel" }, template_id: { type: "string" }, template_vars: { type: "object" } } }, preferred_channel: { type: "string", enum: ["sms", "email", "voice", "auto"], default: "auto" }, send_at_iso: { type: "string", format: "date-time", description: "Schedule for future delivery; omit for immediate" } } }, annotations: { title: "Send Message", readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: true } }, { name: "capture_lead", description: `Structured intake of a prospect into an SMB's funnel with validation, enrichment hooks, and deduplication. Inserts into the SMB's CRM or direct-booking pipeline if available.
...
COST: $0.05 per_lead
LATENCY: ~600ms
EXECUTION: sync_fast (use get_outcome to retrieve result)`, inputSchema: { type: "object", required: ["smb_id", "prospect"], properties: { smb_id: { type: "string" }, prospect: { type: "object", required: ["name"], properties: { name: { type: "string" }, phone: { type: "string" }, email: { type: "string", format: "email" }, service_interest: { type: "string" }, notes: { type: "string" }, consent_record_id: { type: "string", description: "Optional ID of a consent record proving the prospect asked to be contacted (e.g., they filled an SMB's intake form or requested a quote). Required when downstream send_message calls are anticipated." } } }, source: { type: "string", description: "Where the consumer-initiated request originated (e.g., 'consumer_request', 'inbound_quote_form', 'agent_referral_from_find_business')." } } }, annotations: { title: "Capture Lead", readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: false } }, { name: "schedule_appointment", description: `Availability lookup, hold, confirm, reschedule, or cancel appointments with an SMB. Routes through the SMB's native booking system if available, falls back to voice AI or web form.
...
LATENCY: ~5000ms
EXECUTION: async_by_default (use get_outcome to retrieve result)`, inputSchema: { type: "object", required: ["smb_id", "action"], properties: { smb_id: { type: "string" }, action: { type: "string", enum: ["book", "reschedule", "cancel", "check_availability"] }, service: { type: "string" }, customer: { type: "object", properties: { name: { type: "string" }, phone: { type: "string" }, email: { type: "string" } } }, requested_time: { type: "object", properties: { preferred_iso: { type: "string", format: "date-time" }, window_start_iso: { type: "string", format: "date-time" }, window_end_iso: { type: "string", format: "date-time" }, duration_minutes: { type: "integer" } } }, existing_appointment_id: { type: "string", description: "Required for reschedule/cancel" }, notes: { type: "string" } } }, annotations: { title: "Schedule Appointment", readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: true } }, { name: "send_transactional_confirmation", description: 'Idempotent transactional messages: OTPs, booking confirmations, payment receipts, cancellation notices. Guaranteed delivery via redundant channels.\n\nEXAMPLE USER QUERIES THAT MATCH THIS TOOL:\n  user: "Send the booking confirmation receipt to my email"\n  -> call send_transactional_confirmation({"recipient_id": "user@example.com", "channel_preference": "email", "confirmation_type": "booking"})\n\nWHEN TO USE: Use for any message that MUST be delivered reliably \u2014 OTPs, booking confirmations, receipts. Do not use for marketing.\nWHEN NOT TO USE: Do not use for marketing or promotional messages. Do not use for conversational messages.\nCOST: $0.02 per_message\nLATENCY: ~500ms\nEXECUTION: sync_fast (use get_outcome to retrieve result)', inputSchema: { type: "object", required: ["recipient", "confirmation_type", "data"], properties: { recipient: { type: "object", required: ["phone_or_email"], properties: { phone_or_email: { type: "string" }, name: { type: "string" } } }, confirmation_type: { type: "string", enum: ["otp", "booking_confirmation", "payment_receipt", "cancellation_notice", "reminder"] }, data: { type: "object", description: "Type-specific payload; e.g., {otp_code} for otp, {appointment_time, smb_name} for booking_confirmation" }, preferred_channel: { type: "string", enum: ["sms", "email", "auto"], default: "sms" } } }, annotations: { title: "Send Transactional Confirmation", readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: false } }, { name: "handle_inbound", description: `Receive, classify, and route inbound messages on behalf of an SMB. Classifies intent (booking request, cancellation, inquiry, complaint), enriches with context, and routes to the appropriate handler or escalation path.
...
LATENCY: ~3000ms
EXECUTION: async_by_default (use get_outcome to retrieve result)`, inputSchema: { type: "object", required: ["smb_id", "inbound_channel", "raw_message"], properties: { smb_id: { type: "string" }, inbound_channel: { type: "string", enum: ["sms", "email", "voice_voicemail", "web_form", "api"] }, sender: { type: "object", properties: { phone: { type: "string" }, email: { type: "string" }, name: { type: "string" } } }, raw_message: { type: "string" }, received_at_iso: { type: "string", format: "date-time" }, routing_rules: { type: "object", description: "Optional override routing policy for this SMB" } } }, annotations: { title: "Handle Inbound", readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: false } }, { name: "escalate_to_human", description: `Hand off an in-flight task to a human operator with a full context bundle: transcript, prior actions, identifiers, and a recommended next step.
...
COST: $0.005 per_call
LATENCY: ~600ms`, inputSchema: { type: "object", required: ["booking_url"], properties: { booking_url: { type: "string", format: "uri", description: "Full URL the user supplied. Must point at one of the 12 supported booking platforms; auto-detected from the host." }, business_name: { type: "string", description: "Optional override. If omitted, the business name is auto-extracted from the page's <title> or og:title." }, vertical: { type: "string", enum: ["personal_services", "home_services", "professional_services", "restaurants", "retail", "healthcare", "fitness"], description: "Best-guess vertical. If omitted, inferred from the platform (e.g., Doctolib -> healthcare, OpenTable -> restaurants)." }, country_code: { type: "string", description: "ISO 3166-1 alpha-2 (e.g. 'US', 'FR'). Used for compliance routing on later send_message calls." }, contact_phone: { type: "string", description: "Optional. If omitted, the platform integration handles outreach." }, contact_email: { type: "string", description: "Optional." }, capabilities: { type: "array", items: { type: "string" }, description: "Free-form capability tags (e.g., ['haircut','color','blowdry'])." } } }, annotations: { title: "Import Booking Url", readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false } }, { name: "call_business", description: "Place a conversational voice-AI phone call to a business on a consumer's behalf and return a structured answer. THE differentiated capability: reach the ~60M long-tail SMBs that have NO API and NO booking page \u2014 only a phone number. An AI agent cannot pick up a phone and hold a conversation; this tool does. Give a plain-language objective; the voice AI navigates the call and extracts the answer. Business-directed (B2B), far less restricted than calling consumers \u2014 but the compliance gate still enforces recording consent per jurisdiction. Async: returns a call handle; poll get_outcome for the transcript + extracted fields.\n\nWHEN TO USE: Use when the target business has NO booking URL and NO API \u2014 only a phone number \u2014 and the consumer asked the agent to reach them (e.g. 'call this plumber and ask if they can come Tuesday', 'ask the salon if they take walk-ins this afternoon'). Also use to confirm details a booking page doesn't expose (real-time availability, custom quotes).\nWHEN NOT TO USE: Do NOT use when the business has a booking URL \u2014 use import_booking_url + schedule_appointment (cheaper, faster, deterministic). Do NOT use for calls to consumers/individuals (this tool is for reaching businesses). Do NOT use for marketing or telemarketing \u2014 the compliance gate and the B2B-only framing reject that.\nCOST: $0.5 per_call\nLATENCY: ~45000ms\nEXECUTION: async_by_default (use get_outcome to retrieve result)", inputSchema: { type: "object", required: ["objective"], properties: { business_phone: { type: "string", description: "Business phone in E.164 (e.g. +14045550123). Provide this OR smb_id." }, smb_id: { type: "string", description: "Known SMB identifier with a phone on record. Provide this OR business_phone." }, objective: { type: "string", description: "What the call should accomplish, in plain language." }, extract_fields: { type: "array", items: { type: "string" }, description: "Structured fields to pull from the answer, e.g. ['available_tomorrow','price_quote','earliest_slot']." }, country_code: { type: "string", description: "ISO 3166-1 alpha-2 for compliance + recording-consent routing." }, on_behalf_of: { type: "string", description: "Name of the consumer the call is placed for." }, max_duration_seconds: { type: "integer", maximum: 600, default: 180 } } }, annotations: { title: "Call Business", readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: true } }, { name: "check_compliance", description: `Free, instant pre-flight for the compliance gate. Runs the SAME TCPA / GDPR / CASL / CAN-SPAM / 10DLC gate that send_message and call_business run \u2014 but in preview mode, so NO message is sent and NO state changes. Tells you whether a (recipient, channel, message_type, content) send would be permitted BEFORE you pay for it, and if not, names the exact rule and how to remediate. Use it to de-risk a paid send the same way check_booking_link de-risks a paid booking.
...
COST: free
LATENCY: ~15ms`, inputSchema: { type: "object", required: ["recipient_id", "content"], properties: { recipient_id: { type: "string", description: "Phone in E.164 (e.g. '+14045550100') or email address the message would go to." }, content: { type: "string", description: "The actual message body you intend to send. The gate classifies the real text, so a meaningful preview needs the real content." }, channel: { type: "string", enum: ["sms", "email", "voice"], description: "Delivery channel. Omit to auto-infer sms/email from recipient_id; set 'voice' explicitly." }, message_type: { type: "string", description: "Intent tag: transactional, marketing, reminder, follow_up, notification. 'marketing' triggers the consent checks. Defaults to transactional.", default: "transactional" }, country_code: { type: "string", description: "ISO 3166-1 alpha-2 (e.g. 'US', 'DE', 'CA'). Auto-inferred from phone if omitted; drives which jurisdiction rules apply." }, state_code: { type: "string", description: "US state code (e.g. 'CA') for state-specific rules." } } }, annotations: { title: "Check Compliance", readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false } }, { name: "verify_company_record", description: 'Free, live lookup of a company official registry record. Queries the GLEIF global LEI registry (primary, 2.6 million legal entities worldwide) and SEC EDGAR (US public companies) to return the official legal name, LEI, entity status, jurisdiction, registered address, and registry authority. Never fabricates: if the company is not found in these free registries, returns an honest not_found with the sources that were queried.\n\nEXAMPLE USER QUERIES THAT MATCH THIS TOOL:\n  user: "Is Apple Inc a real registered company?"\n  -> call verify_company_record({"name": "Apple Inc", "country": "US"})\n  user: "Look up the LEI for Volkswagen AG"\n  -> call verify_company_record({"name": "Volkswagen AG", "country": "DE"})\n  user: "Verify this LEI: 529900HNOAA1KXQJUQ27"\n  -> call verify_company_record({"name": "Volkswagen AG", "lei": "529900HNOAA1KXQJUQ27"})\n\nWHEN TO USE: Use when you need to verify that a company exists as a registered legal entity and retrieve its official registry details -- before signing a contract, qualifying a vendor, validating a counterparty, or populating a due-diligence record. Accepts a legal name plus optional country filter or a direct LEI for a precise lookup.\nWHEN NOT TO USE: Do not use to verify private companies not registered with GLEIF or SEC. Do not use as an exhaustive fraud-detection tool; this is a first-pass existence check against free public registries, not a full KYC screen.\nCOST: free\nLATENCY: ~800ms', inputSchema: { type: "object", required: ["name"], properties: { name: { type: "string", description: "Legal company name to look up, e.g. Apple Inc or Volkswagen AG." }, country: { type: "string", description: "Optional ISO 3166-1 alpha-2 country filter (e.g. US, DE, GB). Narrows GLEIF results to one jurisdiction." }, lei: { type: "string", description: "Optional 20-character Legal Entity Identifier for a direct, precise lookup." } } }, annotations: { title: "Verify Company Record", readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false } }] } };
