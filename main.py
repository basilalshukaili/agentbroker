...
async def lifespan(app: FastAPI):
    ...
    # NO SANCTIONS-LIST WARM-UP HERE. It was added and REMOVED within the hour.
    #
    # Preloading OFAC + EU + UK at boot meant ~75MB of downloads plus the parsed
    # indexes resident before the first request. On this instance that was an
    # OOM: the process entered a restart loop and the origin served 502 to
    # everything. Deploy status said "live" the whole time - the container was
    # starting fine and then being killed.
    #
    # The lists load lazily instead, and screen_sanctions reports any list that
    # is not yet loaded as NOT screened on that call rather than blocking.
    # Faster boot for a customer is not worth a service that does not boot.
