...
empty search path with schema-qualified tables, revokes inherited PUBLIC/anon/
authenticated execution and grants execution to `service_role`. Existing
...
deliberately granting EXECUTE to PUBLIC, verifies the revoke and an actual
denied-role call, confirms the service-role path, tests exact and mismatched
...
This is one release slice. The other billing `SECURITY DEFINER` RPCs still need
their own execution-grant and owner-authority review. The target database's
